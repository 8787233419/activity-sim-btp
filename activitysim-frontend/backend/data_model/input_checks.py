...
class Household(pa.DataFrameModel):
    ...
    def check_home_zone_in_landuse(cls, households: pd.DataFrame):
        ...
        mask = households.home_zone_id.isin(land_use.zone_id)
        if not mask.all():
            missing = households.loc[~mask, "home_zone_id"].unique()
            hh_min = int(households.home_zone_id.min())
            hh_max = int(households.home_zone_id.max())
            lu_min = int(land_use.zone_id.min()) if len(land_use) else "N/A"
            lu_max = int(land_use.zone_id.max()) if len(land_use) else "N/A"
            log_info(
                f"Zone ID mismatch detected! "
                f"{len(missing)} unique home_zone_id value(s) missing from landuse. "
                f"households home_zone_id range: [{hh_min}, {hh_max}], "
                f"landuse zone_id range: [{lu_min}, {lu_max}]. "
                f"This usually means households use MAZ IDs but landuse uses TAZ IDs (or vice versa). "
                f"Ensure both files use the same zone system."
            )
        return mask
...
class Landuse(pa.DataFrameModel):
    ...
    DISTRICT: int = pa.Field(ge=-1)
    SD: int = pa.Field(ge=-1)
